...
class DialTest extends TestCase
{
    public function testHasFace(): void
    {
        ...
        $this->assertEquals(0, $dial->getFaceValue());
    ...
    public function testCanMove()
    {
        ...
        $this->assertEquals(11, $dial->getFaceValue());

        $dial->movePointer("R8");
        $this->assertEquals(19, $dial->getFaceValue());

        $dial->movePointer("L19");
        $this->assertEquals(0, $dial->getFaceValue());

        $dial->movePointer("L1");
        $this->assertEquals(99, $dial->getFaceValue());

        $dial->movePointer("R1");
        $this->assertEquals(0, $dial->getFaceValue());

        $dial->setPointerAtFaceValue(5);
        $dial->movePointer("L10");
        $this->assertEquals(95, $dial->getFaceValue());
